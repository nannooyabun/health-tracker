...
// ══════════════════════════════════════
// ── Range Calendar Picker (start/end) ──
// ══════════════════════════════════════
function RangeCalendarPicker({ startDate, endDate, onChangeStart, onChangeEnd, onClose, records }) {
  ...
  const handlePick = (dateStr) => {
    if (picking === "start") {
      ...
      if (endDate && dateStr > endDate) onChangeEnd(dateStr);
      setPicking("end");
    } else {
      if (dateStr < startDate) { onChangeStart(dateStr); onChangeEnd(startDate); }
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          {[{ id: "start", label: "開始日" }, { id: "end", label: "終了日" }].map((t) => (
            <button key={t.id} onClick={() => setPicking(t.id)} style={{
              ...
              borderColor: picking === t.id ? "#2e7d32" : "#e0e0e0",
              background: picking === t.id ? "#e8f5e9" : "white",
              fontSize: 16, fontWeight: 700, cursor: "pointer",
              color: picking === t.id ? "#2e7d32" : "#888",
            }}>
              {t.label}<br />
              <span style={{ fontSize: 14, fontWeight: 600 }}>
                {t.id === "start" ? (startDate ? formatDateCompact(startDate) : "未選択") : (endDate ? formatDateCompact(endDate) : "未選択")}
              </span>
            </button>
          ))}
        </div>
          ...
          {days.map((day, idx) => {
            ...
            return (
              <button key={dateStr} onClick={() => handlePick(dateStr)} style={{
                ...
                <span style={{ fontSize: 15, fontWeight: (isStart || isEnd) ? 800 : 600, color: (isStart || isEnd) ? "white" : dow === 0 ? "#e53935" : dow === 6 ? "#1e88e5" : "#444" }}>{day}</span>
              </button>
            );
          })}
        </div>
...
function BPInput({ value, onChange, min, max, label, unit, accentColor, bgColor }) {
  ...
  const startEdit = () => { setEditVal(String(value)); setEditing(true); setTimeout(() => inputRef.current?.focus(), 50); };
  const commit = () => { const v = parseInt(editVal); if (!isNaN(v)) onChange(clamp(v)); setEditing(false); };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, flex: 1, minWidth: 80, maxWidth: 110 }}>
      ...
      {editing ? (
        <input ref={inputRef} type="number" inputMode="numeric" pattern="[0-9]*" value={editVal} onChange={(e) => setEditVal(e.target.value)} onBlur={commit} onKeyDown={(e) => e.key === "Enter" && commit()}
    ...
    <div style={{ background: "white", borderRadius: 24, padding: "20px 16px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", marginBottom: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3 }}>
        {days.map((day, idx) => {
          ...
          const has = !!records[d];
          const hasBP = !!records[d]?.bp;
          const hasMeds = records[d]?.meds && Object.values(records[d].meds).some(Boolean);
          const isT = d === today;
          const dow = idx % 7;
          return (
            <button key={d} onClick={() => onDateTap(d)} style={{
              ...
              {has && (
                <div style={{ display: "flex", gap: 3, marginTop: 2 }}>
                  {hasBP && <div style={{ width: 7, height: 7, borderRadius: "50%", background: isT ? "#81c784" : "#e53935" }} />}
                  {hasMeds && <div style={{ width: 7, height: 7, borderRadius: "50%", background: isT ? "#a5d6a7" : "#1e88e5" }} />}
                </div>
              )}
            </button>
          );
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 12, justifyContent: "center", fontSize: 14, color: "#888" }}>
        ...
        <span style={{ fontSize: 13 }}>※ 全日付タップ可</span>
      </div>
    </div>
  );
}
...
export default function HealthTracker() {
  ...
  const [editDate, setEditDate] = useState(todayStr()); // currently editing date
  ...
  const [periodPreset, setPeriodPreset] = useState("twoweek"); // "week","twoweek","month","threemonth","custom"
  ...
  useEffect(() => {
    ...
    setData(d);
    loadDateRecord(d, todayStr());
    setLoading(false);
  }, []);

  const loadDateRecord = (d, date) => {
    const rec = d.records[date];
    if (rec?.bp) {
      setSystolic(rec.bp.systolic);
      setDiastolic(rec.bp.diastolic);
      setPulse(rec.bp.pulse || 70);
    } else {
      setSystolic(130); setDiastolic(80); setPulse(70);
    }
    setMemo(rec?.memo || "");
  };
  ...
  const switchDate = (date) => {
    setEditDate(date);
    if (data) loadDateRecord(data, date);
  };
  ...
  if (loading) {
    return (<div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#fafaf7", fontFamily: "'Noto Sans JP', sans-serif" }}><div style={{ fontSize: 24, color: "#888" }}>読み込み中...</div></div>);
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #f0f7f0 0%, #fafaf7 30%)", fontFamily: "'Noto Sans JP', sans-serif", paddingBottom: 100 }}>
      ...
      {showRangePicker && <RangeCalendarPicker startDate={customStart} endDate={customEnd} onChangeStart={setCustomStart} onChangeEnd={setCustomEnd} onClose={() => setShowRangePicker(false)} records={data.records} />}

      {/* ── Header (tappable date) ── */}
      <div style={{ background: "linear-gradient(135deg, #2e7d32 0%, #43a047 100%)", padding: "28px 20px 24px", color: "white", borderRadius: "0 0 28px 28px", boxShadow: "0 4px 20px rgba(46,125,50,0.3)" }}>
        ...
        <button onClick={() => setShowDatePicker(true)} style={{
          ...
          borderRadius: 16, padding: "10px 20px", marginTop: 8, cursor: "pointer",
          display: "flex", alignItems: "center", gap: 10, WebkitTapHighlightColor: "transparent",
        }}>
          <span style={{ fontSize: 32, fontWeight: 900, color: "white", letterSpacing: 2 }}>{formatDateFull(editDate)}</span>
          <span style={{ fontSize: 18, color: "rgba(255,255,255,0.8)" }}>▼</span>
        </button>
        <div style={{ fontSize: 20, marginTop: 6, opacity: 0.85, fontWeight: 600, display: "flex", alignItems: "center", gap: 10 }}>
          {getDow(editDate)}曜日
          {!isToday && <span style={{ fontSize: 14, background: "rgba(255,255,255,0.2)", padding: "2px 10px", borderRadius: 8 }}>過去の日付を編集中</span>}
        </div>
      </div>
      ...
      <div style={{ padding: "0 16px", marginTop: 20 }}>
        ...
        {tab === "today" && (
          <>
            {/* Date indicator */}
            {!isToday && (
              <div style={{ background: "#fff3e0", borderRadius: 16, padding: "12px 18px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 17, fontWeight: 700, color: "#e65100" }}>📝 {formatDateFull(editDate)} の記録を編集中</span>
                <button onClick={() => switchDate(today)} style={{ fontSize: 14, padding: "6px 12px", borderRadius: 10, border: "none", background: "#2e7d32", color: "white", fontWeight: 700, cursor: "pointer" }}>今日に戻る</button>
              </div>
            )}
              ...
              {data.medications.map((med) => {
                ...
                return (
                  <button key={med} onClick={() => toggleMed(med)} style={{
                    ...
                    background: checked ? "#f1f8e9" : "#fafafa", cursor: "pointer", WebkitTapHighlightColor: "transparent",
                  }}>
            ...
            <div style={{ background: "white", borderRadius: 24, padding: "24px 20px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", marginBottom: 20 }}>
              ...
              {editRecord.bp && <div style={{ textAlign: "center", marginTop: 12, fontSize: 15, color: "#999" }}>最終記録: {editRecord.bp.time}</div>}
            </div>

            {/* Memo */}
            <div style={{ background: "white", borderRadius: 24, padding: "24px 20px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", marginBottom: 20 }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#1a1a2e", marginBottom: 12 }}>📝 メモ</div>
              <textarea
                value={memo} onChange={(e) => setMemo(e.target.value)} onBlur={saveMemo}
                placeholder="体調やメモを自由に入力..."
                rows={4}
                style={{
                  width: "100%", fontSize: 18, padding: "14px 16px", borderRadius: 16,
                  border: "2px solid #e0e0e0", outline: "none", resize: "vertical",
                  fontFamily: "'Noto Sans JP', sans-serif", lineHeight: 1.6,
                  background: "#fafafa",
                }}
              />
              <div style={{ fontSize: 14, color: "#bbb", marginTop: 6 }}>※ 入力欄の外をタップすると自動保存されます</div>
            </div>
          </>
        )}
        ...
        {tab === "chart" && (
          <>
            ...
            <div style={{ background: "white", borderRadius: 24, padding: "20px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", marginBottom: 20 }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: "#1a1a2e", marginBottom: 12 }}>📆 表示期間</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {Object.entries(presetLabels).map(([key, label]) => (
                  <button key={key} onClick={() => { setPeriodPreset(key); if (key === "custom") setShowRangePicker(true); }}
                    style={{
                      flex: key === "custom" ? "1 1 100%" : 1, padding: "10px 2px", borderRadius: 12, border: "2px solid",
                      fontSize: 16, fontWeight: 700, cursor: "pointer",
                      borderColor: periodPreset === key ? "#2e7d32" : "#e0e0e0",
                      background: periodPreset === key ? "#e8f5e9" : "white",
                      color: periodPreset === key ? "#2e7d32" : "#888",
                      WebkitTapHighlightColor: "transparent",
                    }}>{label}</button>
                ))}
              </div>
              {periodPreset === "custom" && (
                <button onClick={() => setShowRangePicker(true)} style={{
                  marginTop: 10, width: "100%", padding: "10px", borderRadius: 12, border: "2px solid #2e7d32",
                  background: "#f1f8e9", fontSize: 16, fontWeight: 600, color: "#2e7d32", cursor: "pointer",
                }}>
            ...
            <div style={{ background: "white", borderRadius: 24, padding: "20px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", marginBottom: 20 }}>
              ...
              {avgCount === 0 ? (
                ...
                <>
                  <div style={{ textAlign: "center", marginBottom: 16, padding: "12px", borderRadius: 16, background: avgCat.bg }}>
                  ...
                  <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                    {[{ l: "最高血圧", v: avgSys, c: "#e53935", bg: "#fce4ec", u: "mmHg" }, { l: "最低血圧", v: avgDia, c: "#1e88e5", bg: "#e3f2fd", u: "mmHg" }, { l: "脈拍", v: avgPulse, c: "#ff9800", bg: "#fff3e0", u: "bpm" }].map((i) => (
                      <div key={i.l} style={{ flex: 1, background: i.bg, borderRadius: 16, padding: "16px 8px", textAlign: "center" }}>
                        ...
                        <div style={{ fontSize: 13, color: "#999" }}>{i.u}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ textAlign: "center", fontSize: 15, color: "#aaa" }}>計測回数: {avgCount}回</div>
                </>
              )}
            </div>
          ...
          <div style={{ background: "white", borderRadius: 24, padding: "24px 20px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
            ...
            {historyDates.length === 0 ? (
              ...
              historyDates.map((date) => {
                ...
                return (
                  <div key={date} style={{ padding: "16px 18px", marginBottom: 10, borderRadius: 16, background: "#f8f9fa", border: isT ? "2px solid #4caf50" : "2px solid transparent" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <span style={{ fontSize: 20, fontWeight: 700, color: "#333" }}>
                        {formatDateFull(date)}
                        {isT && <span style={{ fontSize: 13, background: "#e8f5e9", color: "#2e7d32", padding: "2px 10px", borderRadius: 10, marginLeft: 8, fontWeight: 700 }}>今日</span>}
                      </span>
                      <button onClick={() => { switchDate(date); setTab("today"); }} style={{
                        fontSize: 14, padding: "6px 14px", borderRadius: 10, border: "2px solid #2e7d32",
                        background: "white", color: "#2e7d32", cursor: "pointer", fontWeight: 700,
                        WebkitTapHighlightColor: "transparent",
                      }}>✏️ 編集</button>
                    </div>
                    {rec.bp && (
                      <div style={{ display: "flex", gap: 12, marginBottom: 8, flexWrap: "wrap", alignItems: "baseline" }}>
                        ...
                        <span style={{ fontSize: 26, fontWeight: 800, color: "#e53935" }}>{rec.bp.systolic}</span>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: rec.memo ? 8 : 0 }}>
                      ...
                      {data.medications.map((m) => {
                        ...
                        return (<span key={m} style={{ fontSize: 15, padding: "3px 10px", borderRadius: 8, background: taken ? "#e8f5e9" : "#f5f5f5", color: taken ? "#2e7d32" : "#ccc", fontWeight: 600 }}>{taken ? "✓" : "✗"} {m}</span>);
                      })}
                    </div>
                    {rec.memo && (
                      <div style={{ background: "#fffde7", borderRadius: 12, padding: "10px 14px", fontSize: 16, color: "#666", lineHeight: 1.5, marginTop: 4 }}>
                        📝 {rec.memo}
                      </div>
                    )}
                  </div>
